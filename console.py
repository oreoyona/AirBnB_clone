#!/usr/bin/python3
"""Module console
This Module defines the console. it
is the entry plint of our project"""
...
class HBNBCommand(cmd.Cmd):
    """The Entey plint of our project"""
    ...
    def do_EOF(self, line):
        """Exists the console when the user
        types ctrl + DD"""
        print()
        return True

    def emptyline(self):
        """prevents the default behavior of cmd"""
        pass

    def do_create(self, line):
        """creates a new object and saves it"""
        tmp = self.class_getter(line)
        if tmp is not None:
            new_obj = tmp()
            new_obj.save()
            print(new_obj.id)
    ...
    def do_show(self, line):
        """prints the string representation of an obj"""
        key = self.key_getter(line)
        if key is None:
            return
    ...
    def do_count(self, line):
        ...
        obj_cls = self.class_getter(line)
        if obj_cls is None:
            return
        result = list(filter(lambda item: isinstance(
            item, obj_cls), storage.all().values()))
    ...
    def key_getter(self, line):
        """method to parse an object key from input"""
        obj_cls = self.class_getter(line)
        if obj_cls is None:
            return None
        id = self.id_getter(line)
        if id is None:
            return None
        return f"{obj_cls.__name__}.{id}"

    def class_getter(self, line):
    ...
    def id_getter(self, line):
        """method to get a class id from input"""
        cmds = line.split()
        if len(cmds) < 2:
